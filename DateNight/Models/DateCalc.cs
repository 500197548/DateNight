...
public class DateCalc
{
    ...
    public string GetTotalCost()
    {
        ...
        {
            if (!string.IsNullOrEmpty(CoffeeCost)) { throw new Exception("Invalid Coffee Cost"); }
        }
        if (!Decimal.TryParse(DinnerCost, out decDinner))
        {
            if (!string.IsNullOrEmpty(DinnerCost)) { throw new Exception("Invalid Dinner Cost"); }
        }
        if (!Decimal.TryParse(MovieCost, out decMovie))
        {
            if (!string.IsNullOrEmpty(MovieCost)) { throw new Exception("Invalid Movie Cost"); }
        }
